...
export default function SeatSlot({player,index,players,me,pos,turnLeft,isShowdown,winners,finished,onKick,state}){
  ...
  return <div style={style} data-seat-index={index} data-seat-rel={rel} data-player-id={player.id} data-cosmetic={player.cosmetic||'default'} className={`seatSlot v2SeatSlot seatSlot${index} ${me?.id===player.id?'heroSlot':''} ${nearHero?'nearHeroSlot':''} ${player.turn?'turnSlot':''} ${player.folded?'foldedSlot':''} ${player.eliminated?'outSlot':''} ${player.sittingOut?'sittingOutSlot':''} ${allIn?'allInSlot':''} ${won?'winnerSlot':''} ${showdownLoser?'loserSlot':''}`}>
    <div className={`seat seatPlate productionSeat seat${index} ${me?.id===player.id?'heroSeat':''} ${player.turn?'turn':''} ${player.folded?'folded':''} ${player.eliminated?'out':''} ${player.sittingOut?'sittingOutSeat':''} ${allIn?'allInSeat':''} ${won?'showdownWinner':''} ${showdownLoser?'showdownLoser':''}`}>
      ...
      {pos[index]&&<div className={`positionBadge p${pos[index].replace('/','')}`}>{pos[index]}</div>}{player.sittingOut&&<span className="awayBadge">AWAY</span>}
