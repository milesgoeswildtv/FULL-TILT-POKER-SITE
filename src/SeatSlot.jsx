...
export default function SeatSlot({player,index,players,me,pos,turnLeft,isShowdown,winners,finished,onKick,state}){
  const geo=seatGeometry(index,players,me),n=Math.max(1,players.length),rel=Number(geo['--seat-rel']),nearHero=rel===1||rel===n-1,won=winners.has(player.id),mucked=isShowdown&&(state.lastResult?.mucked||[]).some(x=>x.playerId===player.id),showdownLoser=isShowdown&&player.cards?.some(Boolean)&&!won,allIn=player.chips===0&&!player.eliminated&&state.started,backAsset=cardBackFor(player),showCards=state.started&&!player.eliminated&&me?.id!==player.id&&!mucked,cardValues=isShowdown&&Array.isArray(player.cards)&&player.cards.some(Boolean)?player.cards.slice(0,2):['',''],initials=player.testBot?'BOT':player.name.split(/\s+/).slice(0,2).map(part=>part[0]).join('').toUpperCase(),style={...geo,'--plaque-art':`url(${plaqueFor(player)})`,'--avatar-frame':`url(${avatarFrameFor(player)})`};
  return <div style={style} data-seat-index={index} data-seat-rel={rel} data-player-id={player.id} data-cosmetic={player.cosmetic||'default'} className={`seatSlot v2SeatSlot seatSlot${index} ${me?.id===player.id?'heroSlot':''} ${nearHero?'nearHeroSlot':''} ${player.turn?'turnSlot':''} ${player.folded?'foldedSlot':''} ${player.eliminated?'outSlot':''} ${player.sittingOut?'sittingOutSlot':''} ${mucked?'muckedSlot':''} ${allIn?'allInSlot':''} ${won?'winnerSlot':''} ${showdownLoser?'loserSlot':''}`}>
    <div className={`seat seatPlate productionSeat seat${index} ${me?.id===player.id?'heroSeat':''} ${player.turn?'turn':''} ${player.folded?'folded':''} ${player.eliminated?'out':''} ${player.sittingOut?'sittingOutSeat':''} ${mucked?'muckedSeat':''} ${allIn?'allInSeat':''} ${won?'showdownWinner':''} ${showdownLoser?'showdownLoser':''}`}>
      ...
      {pos[index]&&<div className={`positionBadge p${pos[index].replace('/','')}`}>{pos[index]}</div>}{player.sittingOut&&<span className="awayBadge">AWAY</span>}{mucked&&<span className="muckedBadge">MUCKED</span>}
