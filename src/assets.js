...
export const cosmeticOptions=[{key:'default',label:'Crashout Poker'},{key:'constellation',label:'Constellation'},{key:'deadMansHand',label:"Dead Man's Hand"},{key:'regalia',label:'Regalia'}];
export function normalizeCosmetic(value){return cosmeticOptions.some(x=>x.key===value)?value:'default'}
export function skinFor(player){const key=normalizeCosmetic(player?.cosmetic);return key==='default'?{label:'Crashout Poker',avatarFrame:assets.default.avatarFrame,cardBack:assets.default.starterCardBack,plaques:assets.default.plaques,chips:assets.default.chips}:assets.cosmetics[key]}
