...
test('linking migrates Telegram history into the Discord canonical account exactly once',async()=>{
 ...
 const target=await post(ns.get(discord.id),'/notifications/telegram-target');assert.equal(target.body.telegram.chatId,'99887766');assert.equal(target.body.telegram.username,'samtelegram');
