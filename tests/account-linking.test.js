...
test('linking migrates Telegram history into the Discord canonical account exactly once',async()=>{
 ...
 const preferences=await post(ns.get(discord.id),'/notifications/preferences',{notifications:{playerJoined:false,turnReminder:true}});assert.equal(preferences.response.status,200);assert.equal(preferences.body.account.notifications.playerJoined,false);assert.equal(preferences.body.account.notifications.turnReminder,true);
 const target=await post(ns.get(discord.id),'/notifications/telegram-target');assert.equal(target.body.telegram.chatId,'99887766');assert.equal(target.body.telegram.username,'samtelegram');assert.equal(target.body.notifications.playerJoined,false);assert.equal(target.body.notifications.turnReminder,true);
 let telegramProfile=await accountRequest(env,telegram,'/profile');assert.equal(telegramProfile.account.notifications.playerJoined,false);assert.equal(telegramProfile.account.notifications.turnReminder,true);
