...
async function initTwo(c){const r=await c.fetch(new Request('https://tournament/init',{method:'POST',headers:{'content-type':'application/json'},body:JSON.stringify({code:'RACE01',players:[{name:'Host',accountId:'acct-host'},{name:'Guest',accountId:'acct-guest'}],startingChips:2500,blindStructure:[[50,100],[75,150]],levelDurationMs:60000,now:1000})}));assert.equal(r.status,201)}
...
test('start locks registration before asynchronous child provisioning can finish',async()=>{
 ...
 h.releaseProvision();const started=await startPromise,j=await started.json();assert.equal(started.status,200);assert.equal(j.status,'running');assert.equal(j.controlTransition,null);assert.equal(j.players.length,2);assert.equal(h.coordinator.data.telegramNotifications.filter(x=>x.kind==='tournament-start').length,2);
});
...
test('pause and resume cannot interleave and completed controls are idempotent',async()=>{
 ...
 const pausedAt=h.coordinator.data.clock.pausedAt,nextBreakBefore=h.coordinator.data.breakSchedule.nextDueAt;r=await h.coordinator.fetch(new Request('https://tournament/pause',{method:'POST',headers:{'content-type':'application/json'},body:JSON.stringify({now:4500})}));assert.equal(r.status,200);assert.equal(h.coordinator.data.clock.pausedAt,pausedAt);
 r=await h.coordinator.fetch(new Request('https://tournament/resume',{method:'POST',headers:{'content-type':'application/json'},body:JSON.stringify({now:5000})}));assert.equal(r.status,200);assert.equal((await r.json()).status,'running');assert.equal(h.coordinator.data.breakSchedule.nextDueAt,nextBreakBefore+2000);const levelStarted=h.coordinator.data.clock.levelStartedAt;
