...
function chooseAction(table,p){const currentBet=table.betToMatch(),toCall=table.toCall(p),raiseOpen=table.raiseIsOpen(p),hasOpponent=table.hasRaiseOpponent(p),canAllIn=p.bet+p.chips<=currentBet||(hasOpponent&&raiseOpen);if(canAllIn)return'allin';return toCall>0?'call':'check'}
