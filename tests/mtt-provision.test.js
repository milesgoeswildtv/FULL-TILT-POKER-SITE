...
test('child payload preserves tournament player identity, stack and token',()=>{const tournament={code:'MOVE01',startingChips:5000,players:[{id:'p01',token:'secret',name:'Sam',chips:8125,seat:3,moveCount:2}],clock:{blindStructure:[[100,200]],levelDurationMs:300000}},table={tableNumber:4,playerIds:['p01']};const payload=childTablePayload(tournament,table);assert.deepEqual(payload.players[0],{tournamentPlayerId:'p01',token:'secret',name:'Sam',avatarUrl:null,chips:8125,seat:3,moveCount:2})});

test('child payload sorts players into coordinator-assigned seat order',()=>{const tournament={code:'SEATS1',startingChips:2500,players:[{id:'p01',token:'a',name:'One',chips:2500,seat:1,moveCount:0},{id:'p02',token:'b',name:'Two',chips:1800,seat:2,moveCount:2}],clock:{blindStructure:[[50,100]],levelDurationMs:150000}},table={tableNumber:3,playerIds:['p02','p01']};const payload=childTablePayload(tournament,table);assert.deepEqual(payload.players.map(p=>[p.tournamentPlayerId,p.seat,p.chips]),[['p01',1,2500],['p02',2,1800]])});

test('empty child tables cannot be provisioned',()=>{assert.throws(()=>childTablePayload({code:'ABC123',players:[],clock:{blindStructure:[[10,20]],levelDurationMs:60000}}, {tableNumber:1,playerIds:[]}),/empty tournament table/)});
