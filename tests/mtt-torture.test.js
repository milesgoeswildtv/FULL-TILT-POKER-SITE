...
function firstOpenSeat(snapshot,playerId){const used=new Set(snapshot.players.filter(p=>p.id!==playerId&&Number.isInteger(p.seat)&&p.seat>=1).map(p=>p.seat));for(let seat=1;seat<=snapshot.capacity;seat++)if(!used.has(seat))return seat;return null}
...
function buildTournament(seed){
 ...
 const tables=seating.tables.map(t=>({tableNumber:t.tableNumber,tableKey:`SIM${seed}-T${t.tableNumber}`,capacity:MTT_TABLE_CAPACITY,playerIds:t.players.map(p=>p.playerId),status:'running',handNumber:0,reportGeneration:1,lastBoundarySequence:0,lastBoundaryFingerprint:null,lastReportAt:null,nextBigBlindPlayerId:null,provisioned:true}));
...
function syncBoundary(coordinator,childRosters,tableNumber,stats,now){
 ...
 if(incoming.length){for(const move of incoming){const s=coordinator.sessionState(move.playerToken,now).session;assert.equal(s.tableNumber,tableNumber);stats.reconnectChecks++}const acknowledgements=incoming.map(move=>{const seat=move.toSeat??firstOpenSeat(snapshot,move.playerId);assert.ok(Number.isInteger(seat)&&seat>=1&&seat<=snapshot.capacity,'destination must choose a legal move seat');return{id:move.id,seat}}),acked=coordinator.acknowledgeMoves(tableNumber,acknowledgements);assert.equal(acked.length,incoming.length);stats.moveAcks+=acked.length;snapshot=coordinator.tableSnapshot(tableNumber,now);childRosters.set(tableNumber,new Set(snapshot.players.map(p=>p.id)));for(const move of incoming){const p=coordinator.player(move.playerId);assert.equal(p.reportOwnerTable,tableNumber);assert.equal(p.pendingMoveId,null);assert.ok(Number.isInteger(p.seat)&&p.seat>=1&&p.seat<=snapshot.capacity)}}
...
function simulateHand(coordinator,childRosters,tableNumber,rng,stats,now,forceBust=false){
 ...
 const duration=2000+randInt(rng,43001),endNow=now+duration,table=coordinator.table(tableNumber),activeRows=rows.filter(row=>row.chips>0),nextBigBlindPlayerId=activeRows.length>=2?activeRows[randInt(rng,activeRows.length)].id:null,outcome=coordinator.reportTable(tableNumber,{reportGeneration:table.reportGeneration,boundarySequence:table.lastBoundarySequence+1,handNumber:table.handNumber+1,completedAt:endNow,status:'running',nextBigBlindPlayerId,players:rows});assert.equal(outcome.kind,'new');assert.equal(outcome.newlyBusted.length,busts);stats.hands++;stats.eliminations+=busts;
...
function assertInvariants(coordinator,childRosters){
 ...
 const active=players.filter(activePlayer),seen=new Map(),unacked=(coordinator.data.pendingMoves||[]).filter(m=>!m.acknowledged),moveByPlayer=new Map(unacked.map(m=>[m.playerId,m]));assert.equal(moveByPlayer.size,unacked.length,'a player cannot have two live table moves');
 for(const table of coordinator.data.tables){const canonical=activeAt(coordinator,table.tableNumber),seats=new Set();assert.ok(canonical.length<=table.capacity,'table exceeded capacity');for(const p of canonical){assert.equal(p.tableNumber,table.tableNumber);const move=moveByPlayer.get(p.id);if(move?.kind==='balance'&&move.toSeat==null){assert.equal(p.seat,null,'deferred balancing player must remain seatless until destination boundary');continue}assert.ok(Number.isInteger(p.seat)&&p.seat>=1&&p.seat<=table.capacity);if(move)assert.equal(p.seat,move.toSeat,'preseated transit player must retain the canonical destination seat');assert.ok(!seats.has(p.seat),'duplicate canonical seat');seats.add(p.seat)}const roster=childRosters.get(table.tableNumber)||new Set();if(table.status==='closed'){assert.equal(canonical.length,0,'closed table retained active player');assert.equal(roster.size,0,'closed child roster retained player')}for(const id of roster){const p=coordinator.player(id);assert.ok(p&&activePlayer(p),'child roster retained busted player');assert.equal(p.tableNumber,table.tableNumber,'child roster points at stale table');seen.set(id,(seen.get(id)||0)+1)}}
 for(const p of active){const n=seen.get(p.id)||0;if(moveByPlayer.has(p.id))assert.equal(n,0,'in-transit player was dealt at a child table');else assert.equal(n,1,'active player must exist at exactly one child table')}
