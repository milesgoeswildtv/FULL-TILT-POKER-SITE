...
function coordinatorWithSizes(sizes){
 ...
 const c=new TournamentCoordinator({storage:{}},{});c.data={code:'TEST',status:'running',startingChips:2500,players,tables,pendingMoves:[],eliminationLedger:[],clock:createTournamentClock({blindStructure:[[50,100],[75,150]],levelDurationMs:150000,now:1000})};return c;
}
...
test('coordinator owns global elimination places instead of trusting a child table place',()=>{const c=coordinatorWithSizes([2]),p1=c.player('p1'),p2=c.player('p2');const outcome=c.reportTable(1,{reportGeneration:1,boundarySequence:1,handNumber:1,completedAt:2000,status:'running',players:[reportRow(p1,{chips:5000,handStartChips:2500,stats:{handsPlayed:1}}),{...reportRow(p2,{chips:0,handStartChips:2500,stats:{handsPlayed:1}}),eliminated:true,finishPlace:99}]});assert.deepEqual(outcome.newlyBusted,['p2']);assert.equal(c.player('p2').finishPlace,2);assert.equal(c.player('p1').finishPlace,1);assert.equal(c.data.status,'finished')});

test('balance move is scheduled only when its source table reaches a safe boundary',()=>{const c=coordinatorWithSizes([8,6]);assert.deepEqual(c.maybeScheduleMoves(2),[]);const moves=c.maybeScheduleMoves(1),move=moves[0],p=c.player(move.playerId);assert.equal(moves.length,1);assert.equal(move.fromTable,1);assert.equal(move.toTable,2);assert.equal(c.data.pendingMoves.length,1);assert.equal(p.tableNumber,2);assert.equal(p.reportOwnerTable,null);assert.equal(p.pendingMoveId,move.id);assert.equal(p.ownershipGeneration,move.toOwnershipGeneration);assert.equal(c.table(1).reportGeneration,2)});

test('table break relocates every active source player and closes the source table',()=>{const c=coordinatorWithSizes([2,3]);const moves=c.maybeScheduleMoves(1);assert.equal(moves.length,2);assert.equal(c.table(1).status,'closed');assert.equal(c.table(1).playerIds.length,0);assert.equal(c.table(2).playerIds.length,5);assert.equal(c.data.pendingMoves.filter(m=>!m.acknowledged).length,2);assert.ok(moves.every(m=>c.player(m.playerId).reportOwnerTable===null))});

test('destination acknowledgement transfers report ownership without scheduling another move',()=>{const c=coordinatorWithSizes([8,6]),move=c.maybeScheduleMoves(1)[0],beforePending=c.data.pendingMoves.length;assert.equal(c.acknowledgeMoves(2,[move.id]).length,1);assert.equal(c.data.pendingMoves[0].acknowledged,true);const p=c.player(move.playerId);assert.equal(p.reportOwnerTable,2);assert.equal(p.pendingMoveId,null);assert.equal(c.table(2).reportGeneration,2);assert.equal(c.data.pendingMoves.length,beforePending)});
