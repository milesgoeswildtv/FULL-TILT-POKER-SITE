...
export function childTablePayload(tournament,table){
 ...
 return{code:tournamentTableKey(tournament.code,table.tableNumber),tournamentCode:tournament.code,tournamentTableNumber:table.tableNumber,startingChips:tournament.startingChips,blindStructure:tournament.clock.blindStructure,blindMinutes:tournament.clock.levelDurationMs/60000,players:players.map(p=>({tournamentPlayerId:p.id,token:p.token,name:p.name,avatarUrl:p.avatarUrl||null,chips:p.chips,seat:p.seat,moveCount:p.moveCount||0}))};
}
