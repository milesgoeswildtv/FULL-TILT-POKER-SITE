...
export async function handleTournamentApi(req,env){
 ...
 if(u.pathname==='/api/tournaments'&&req.method==='POST'){
  ...
  for(let attempt=0;attempt<CODE_RETRIES;attempt++){
   const tournamentCode=code(),stub=coordinator(env,tournamentCode),result=await stub.fetch(new Request('https://tournament/init',{method:'POST',headers:{'content-type':'application/json'},body:JSON.stringify({code:tournamentCode,players:[{name:hostName,accountId:String(input.accountId||''),avatarUrl:String(input.avatarUrl||''),cosmetic:String(input.cosmetic||'default')}],startingChips:config.startingChips,blindStructure:blindStructure.map(([sb,bb])=>[sb,bb,input.bigBlindAnte?bb:0]),levelDurationMs:config.blindMinutes*60000,bigBlindAnte:!!input.bigBlindAnte,breaksEnabled:input.breaksEnabled!==false,breakEveryMinutes:input.breakEveryMinutes,breakDurationMinutes:input.breakDurationMinutes})}));
 ...
 if(route==='recover'&&req.method==='POST'){const{raw}=await requestBody(req);return forward(stub,'/recover',req,raw)}
