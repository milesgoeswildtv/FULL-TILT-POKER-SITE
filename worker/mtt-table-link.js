...
function cleanAvatarUrl(value){const s=String(value||'').trim();return /^https:\/\//i.test(s)?s.slice(0,2048):''}
function freshPlayer(row){const chips=nonNegativeInt(row.chips);return{id:`mtt-${row.id}`,tournamentPlayerId:row.id,ownershipGeneration:generation(row.ownershipGeneration),accountId:String(row.accountId||'')||null,avatarUrl:cleanAvatarUrl(row.avatarUrl)||null,token:String(row.token||''),name:String(row.name||''),chips,bet:0,contributed:0,host:false,testBot:false,cosmetic:String(row.cosmetic||'default'),sittingOut:!!row.sittingOut,timeBankMs:normalizeTimeBankMs(row.timeBankMs),folded:false,eliminated:!!row.eliminated||chips<=0,finishPlace:Number.isInteger(row.finishPlace)?row.finishPlace:null,cards:[],stats:cleanStats(row.stats),handStartChips:chips,vpipThisHand:false,pfrThisHand:false,moveCount:nonNegativeInt(row.moveCount),tournamentSeat:seatNumber(row.seat)}}
function resetAtBoundary(player,row){const chips=nonNegativeInt(row.chips);player.tournamentPlayerId=row.id;player.ownershipGeneration=generation(row.ownershipGeneration);player.accountId=String(row.accountId||player.accountId||'')||null;player.avatarUrl=cleanAvatarUrl(row.avatarUrl||player.avatarUrl)||null;player.token=String(row.token||player.token||'');player.name=String(row.name||player.name||'');player.chips=chips;player.bet=0;player.contributed=0;player.host=false;player.testBot=!!player.testBot;player.cosmetic=String(row.cosmetic||player.cosmetic||'default');player.sittingOut=!!row.sittingOut;player.timeBankMs=normalizeTimeBankMs(row.timeBankMs,player.timeBankMs);player.folded=false;player.eliminated=!!row.eliminated||chips<=0;player.finishPlace=Number.isInteger(row.finishPlace)?row.finishPlace:null;player.cards=[];player.stats=cleanStats(row.stats||player.stats);player.handStartChips=chips;player.vpipThisHand=false;player.pfrThisHand=false;player.moveCount=nonNegativeInt(row.moveCount);player.tournamentSeat=seatNumber(row.seat);return player}
