...
export function reconcileTournamentRoster(tableData,snapshot){
 ...
 const present=new Map(rows.map(r=>[r.id,generation(r.ownershipGeneration)])),acceptedMoveIds=moves.filter(m=>m?.toTable===Number(snapshot.tableNumber)&&present.get(m.playerId)===generation(m.toOwnershipGeneration)&&m.id).map(m=>m.id);
 return{added,removed,acceptedMoveIds};
}

export function tournamentTableReport(tableData){
 const handNumber=nonNegativeInt(tableData?.handNumber),completedAt=nonNegativeInt(tableData?.lastResult?.finishedAt);
 return{reportGeneration:generation(tableData?.mttReportGeneration),boundarySequence:handNumber,handNumber,completedAt,status:tableData?.street==='finished'?'finished':tableData?.paused?'paused':'running',players:(tableData?.players||[]).map(p=>({id:p.tournamentPlayerId||p.id,ownershipGeneration:generation(p.ownershipGeneration),name:p.name,chips:nonNegativeInt(p.chips),eliminated:!!p.eliminated,finishPlace:Number.isInteger(p.finishPlace)?p.finishPlace:null,handStartChips:nonNegativeInt(p.handStartChips),stats:cleanStats(p.stats),cosmetic:String(p.cosmetic||'default')}))};
}
