...
export function reconcileTournamentRoster(tableData,snapshot){
 ...
 const original=[...(tableData.players||[])],dealer=original[Number(tableData.dealerIndex)||0]||null,dealerId=canonicalId(dealer),dealerSeat=Number(dealer?.tournamentSeat)||null,moves=Array.isArray(snapshot.moves)?snapshot.moves:[],rows=snapshot.players.map(row=>({...row}));
 ...
 rows.sort((a,b)=>(Number(a.seat)||99)-(Number(b.seat)||99));
 ...
 const seatById=new Map(rows.map(r=>[r.id,Number.isInteger(Number(r.seat))?Number(r.seat):null]));
...
export function tournamentTableReport(tableData){
 const handNumber=nonNegativeInt(tableData?.handNumber),completedAt=nonNegativeInt(tableData?.lastResult?.finishedAt),players=tableData?.players||[],dealer=players[Number(tableData?.dealerIndex)||0]||null,positionRows=players.map(p=>({id:canonicalId(p),seat:Number(p.tournamentSeat),chips:nonNegativeInt(p.chips),eliminated:!!p.eliminated})),nextBigBlindPlayerId=computeNextBigBlindPlayerId({players:positionRows,dealerPlayerId:canonicalId(dealer),dealerSeat:Number(dealer?.tournamentSeat)})||null;
