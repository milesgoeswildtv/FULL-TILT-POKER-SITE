...
import{TIME_BANK_INITIAL_MS,normalizeTimeBankMs}from'./time-bank.js';
import{normalizeBreakSchedule,scheduleFirstBreak,scheduledBreakDue,shiftScheduledBreak,nextScheduledBreak,breakCountdown}from'./tournament-breaks.js';
...
function publicState(d,now=Date.now()){
 ...
 const players=d.players.map(({token,accountId,reportOwnerTable,pendingMoveId,ownershipGeneration,...p})=>p),tables=d.tables.map(({lastBoundaryFingerprint,...table})=>table),anteMode=d.anteMode==='big-blind'?'big-blind':'none',breakSchedule=d.breakSchedule?{enabled:!!d.breakSchedule.enabled,everyMs:Number(d.breakSchedule.everyMs)||0,durationMs:Number(d.breakSchedule.durationMs)||0,nextDueAt:Number(d.breakSchedule.nextDueAt)||null,sequence:nonNegativeInt(d.breakSchedule.sequence)}:null,scheduledBreak=d.scheduledBreak?{...d.scheduledBreak,remainingMs:breakCountdown(d.scheduledBreak,now)}:null;
 return{code:d.code,status:d.status,controlTransition,registrationOpen:d.status==='lobby'&&!controlTransition&&!d.tables?.some(t=>t.provisioned),startingChips:d.startingChips,maxPlayers:MTT_MAX_PLAYERS,tableCapacity:MTT_TABLE_CAPACITY,maxTables:MTT_MAX_TABLES,clock,anteMode,breakSchedule,scheduledBreak,fieldRemaining:activeField(d).length,players,tables,pendingMoves,createdAt:d.createdAt,startedAt:d.startedAt||null,finishedAt:d.finishedAt||null,endedAt:d.endedAt||null,endedByHost:!!d.endedByHost};
}
...
export class TournamentCoordinator{
 ...
 async load(){
  ...
  const d=this.data;d.pendingMoves??=[];d.eliminationLedger??=[];d.telegramNotifications??=[];d.endedAt??=null;d.endedByHost=!!d.endedByHost;d.resultsSyncedAt??=null;d.breakSchedule??=normalizeBreakSchedule({breaksEnabled:false});d.anteMode??=(d.clock?.blindStructure?.some(level=>Number(level?.[2])>0)?'big-blind':'none');d.scheduledBreak??=null;d.lastScheduledBreak??=null;if(d.status==='starting')d.transition={type:'start',...(d.transition||{})};else d.transition=null;
  for(const table of d.tables||[]){table.reportGeneration=positiveInt(table.reportGeneration);table.lastBoundarySequence=nonNegativeInt(table.lastBoundarySequence??table.handNumber);table.lastBoundaryFingerprint??=null;table.handNumber=nonNegativeInt(table.handNumber);table.nextBigBlindPlayerId??=null;table.breakBoundaryAt=nonNegativeInt(table.breakBoundaryAt)}
  for(const p of d.players||[]){p.stats={...cleanStats(),...cleanStats(p.stats)};p.host=!!p.host;p.accountId=String(p.accountId||'')||null;p.cosmetic=cleanCosmetic(p.cosmetic);p.sittingOut=!!p.sittingOut;p.timeBankMs=normalizeTimeBankMs(p.timeBankMs);p.ownershipGeneration=positiveInt(p.ownershipGeneration);if(p.reportOwnerTable===undefined)p.reportOwnerTable=p.eliminated?null:p.tableNumber;p.pendingMoveId??=null}
 ...
 tableSnapshot(tableNumber,now=Date.now()){
  ...
  return{tournamentCode:this.data.code,tableNumber:table.tableNumber,tableKey:table.tableKey,capacity:table.capacity,reportGeneration:positiveInt(table.reportGeneration),status:this.data.status,tableStatus:table.status,fieldRemaining:activeField(this.data).length,players,moves,clock:tournamentClockState(this.data.clock,now),handBlinds:blindsForNewHand(this.data.clock,now),anteMode:this.data.anteMode==='big-blind'?'big-blind':'none',breakSchedule:this.data.breakSchedule?{enabled:!!this.data.breakSchedule.enabled,everyMs:Number(this.data.breakSchedule.everyMs)||0,durationMs:Number(this.data.breakSchedule.durationMs)||0,nextDueAt:Number(this.data.breakSchedule.nextDueAt)||null,sequence:nonNegativeInt(this.data.breakSchedule.sequence)}:null,scheduledBreak:this.data.scheduledBreak?{...this.data.scheduledBreak,remainingMs:breakCountdown(this.data.scheduledBreak,now)}:null};
 }
 reportTable(tableNumber,rawReport){
  ...
  const newlyBusted=[];for(const row of report.players){const p=this.player(row.id);if(!p)throw Error('Tournament report player is missing from coordinator state.');const wasAlive=!p.eliminated&&p.chips>0;p.chips=row.chips;p.stats=cleanStats(row.stats||p.stats);p.cosmetic=cleanCosmetic(row.cosmetic||p.cosmetic);p.sittingOut=!!row.sittingOut;p.timeBankMs=normalizeTimeBankMs(row.timeBankMs,p.timeBankMs);p.handStartChips=row.handStartChips;if(wasAlive&&row.chips===0){p.eliminated=true;p.reportOwnerTable=null;p.pendingMoveId=null;newlyBusted.push(p);const event=eliminationEvent({tableNumber,report,player:p});if(!this.data.eliminationLedger.some(e=>e.id===event.id))this.data.eliminationLedger.push(event)}}
  table.lastReportAt=Date.now();table.breakBoundaryAt=Math.max(nonNegativeInt(table.breakBoundaryAt),nonNegativeInt(report.completedAt||Date.now()));table.handNumber=report.handNumber;table.lastBoundarySequence=report.boundarySequence;table.lastBoundaryFingerprint=checked.fingerprint;table.nextBigBlindPlayerId=report.nextBigBlindPlayerId;if(table.status!=='closed')table.status=report.status||table.status||'running';
 ...
 scheduleCoordinatorAlarm(now=Date.now()){if(typeof this.state.storage.setAlarm!=='function'||!this.data)return;const times=[],q=(this.data.telegramNotifications||[]).filter(x=>x.status==='pending');if(q.length)times.push(Math.min(...q.map(x=>Math.max(now+50,Number(x.nextAttemptAt)||now))));if(this.data.status==='break'&&Number(this.data.scheduledBreak?.endsAt)>now)times.push(Number(this.data.scheduledBreak.endsAt));if(times.length)this.state.storage.setAlarm(Math.min(...times))}
 scheduleTelegramNotifications(){this.scheduleCoordinatorAlarm()}
 ...
 async flushTelegramNotifications(now=Date.now()){const q=(this.data?.telegramNotifications||[]).filter(x=>x.status==='pending'&&(Number(x.nextAttemptAt)||0)<=now).slice(0,8);for(const event of q){const p=this.player(event.playerId),result=p?.accountId?await sendTelegramTournamentNotification(this.env,p.accountId,event):{sent:false,retryable:false,reason:'no-account'};event.attempts=nonNegativeInt(event.attempts)+1;event.lastAttemptAt=Date.now();if(result.sent){event.status='delivered';event.deliveredAt=Date.now();event.lastError=null}else if(result.retryable&&event.attempts<5){event.nextAttemptAt=Date.now()+Math.min(30000,1000*(2**Math.max(0,event.attempts-1)));event.lastError=result.reason||'Telegram delivery failed.'}else{event.status='failed';event.failedAt=Date.now();event.lastError=result.reason||'Telegram delivery failed.'}}if(q.length)await this.save();this.scheduleCoordinatorAlarm(now)}
 openBreakTables(){return(this.data?.tables||[]).filter(t=>t.provisioned&&t.status!=='closed'&&t.playerIds.some(id=>{const p=this.player(id);return p&&!p.eliminated&&p.chips>0}))}
 observeBreakBoundary(tableNumber,now=Date.now()){const table=this.table(tableNumber);if(!table)return false;table.breakBoundaryAt=Math.max(nonNegativeInt(table.breakBoundaryAt),nonNegativeInt(now));return true}
 breakTablesReady(){const due=Number(this.data?.scheduledBreak?.dueAt)||0,tables=this.openBreakTables();return!!(due&&tables.length&&tables.every(t=>Number(t.breakBoundaryAt||0)>=due))}
 async maybeStartScheduledBreak(now=Date.now()){const d=this.data;if(!d||!d.breakSchedule?.enabled||activeField(d).length<=1||['finished','ended','paused','break'].includes(d.status))return false;if(d.status==='running'&&scheduledBreakDue(d.breakSchedule,now)){pauseTournamentClock(d.clock,now);d.status='break-pending';d.scheduledBreak={sequence:nonNegativeInt(d.breakSchedule.sequence)+1,dueAt:Number(d.breakSchedule.nextDueAt),pendingAt:now,startedAt:null,endsAt:null};await this.save()}if(d.status!=='break-pending'||!this.breakTablesReady())return false;d.status='break';d.scheduledBreak.startedAt=now;d.scheduledBreak.endsAt=now+Number(d.breakSchedule.durationMs);await this.save();const failures=await this.controlTables('pause',now);d.scheduledBreak.pauseFailures=failures;await this.save();this.scheduleCoordinatorAlarm(now);return true}
 async finishScheduledBreak(now=Date.now()){const d=this.data;if(!d||d.status!=='break'||!d.scheduledBreak||Number(d.scheduledBreak.endsAt)>now)return false;const finished={...d.scheduledBreak,endedAt:now};resumeTournamentClock(d.clock,now);nextScheduledBreak(d.breakSchedule,now);d.status='running';d.lastScheduledBreak=finished;d.scheduledBreak=null;await this.save();const failures=await this.controlTables('resume',now);if(failures.length){const failedAt=Date.now();pauseTournamentClock(d.clock,failedAt);d.status='paused';d.lastScheduledBreak={...finished,resumeFailures:failures};await this.controlTables('pause',failedAt)}await this.save();this.scheduleCoordinatorAlarm(now);return true}
 async alarm(){await this.load();if(!this.data)return;const now=Date.now();if(this.data.status==='break'&&Number(this.data.scheduledBreak?.endsAt)<=now)await this.finishScheduledBreak(now);await this.flushTelegramNotifications(now);this.scheduleCoordinatorAlarm(now)}
 maybeScheduleMoves(boundaryTableNumber){
  if(this.data.status!=='running'||activeField(this.data).length<=1||scheduledBreakDue(this.data.breakSchedule))return[];
 ...
 async fetch(req){
  ...
  if(u.pathname==='/start'&&req.method==='POST'){
   ...
   if(['running','paused','break-pending','break'].includes(this.data.status)&&this.data.startedAt)return json(publicState(this.data));
   ...
   try{
    ...
    const now=Number(this.data.transition?.effectiveAt)||Number(b.now)||Date.now();if(!this.data.transition.effectiveAt){this.data.transition.effectiveAt=now;this.data.startedAt=now;this.data.clock.levelStartedAt=now;this.data.clock.paused=false;this.data.clock.pausedAt=null;scheduleFirstBreak(this.data.breakSchedule,now);this.data.endedAt=null;this.data.endedByHost=false;await this.save()}
  ...
  if(tableMatch&&tableMatch[2]==='sync'&&req.method==='GET'){try{const n=Number(tableMatch[1]),now=Date.now(),atBoundary=u.searchParams.get('atBoundary')==='1';if(atBoundary){this.observeBreakBoundary(n,now);await this.maybeStartScheduledBreak(now)}if(atBoundary&&this.data.status==='running'){const moves=this.maybeScheduleMoves(n);if(moves.length)this.assertTournamentChipSupply()}await this.save();return json(this.tableSnapshot(n,now))}catch(e){return json({error:e.message},404)}}
  if(tableMatch&&tableMatch[2]==='report'&&req.method==='POST'){try{const n=Number(tableMatch[1]),raw=await req.json(),outcome=this.reportTable(n,raw),now=Number(raw?.completedAt)||Date.now();if(outcome.kind==='new')await this.maybeStartScheduledBreak(now);if(outcome.kind==='new'&&this.data.status==='running')this.maybeScheduleMoves(n);if(this.data.status==='finished')this.queueTournamentResultNotifications();await this.save();if(this.data.status==='finished'){this.scheduleTelegramNotifications();await this.syncTournamentResults()}return json(this.tableSnapshot(n,now))}catch(e){return json({error:e.message},400)}}
  ...
  if(u.pathname==='/pause'&&req.method==='POST'){
   ...
   this.transitionInFlight='pause';try{const b=await req.json().catch(()=>({})),now=Number(b.now)||Date.now();if(this.data.status==='running'){pauseTournamentClock(this.data.clock,now);this.data.manualPauseStartedAt=now}this.data.status='paused';this.data.transition={type:'pause',startedAt:now};await this.save();const failures=await this.controlTables('pause',now);this.data.transition=null;await this.save();return json({...publicState(this.data,now),childFailures:failures},failures.length?503:200)}finally{this.transitionInFlight=null}
  }
  if(u.pathname==='/resume'&&req.method==='POST'){
   ...
   this.transitionInFlight='resume';try{const b=await req.json().catch(()=>({})),now=Number(b.now)||Date.now();if(this.data.status==='paused'){const pausedFor=Math.max(0,now-Number(this.data.clock?.pausedAt||now));shiftScheduledBreak(this.data.breakSchedule,pausedFor);resumeTournamentClock(this.data.clock,now);this.data.manualPauseStartedAt=null}this.data.status='running';this.data.transition={type:'resume',startedAt:now};await this.save();const failures=await this.controlTables('resume',now);if(failures.length){const failedAt=Date.now();pauseTournamentClock(this.data.clock,failedAt);this.data.status='paused';this.data.transition={type:'pause',startedAt:failedAt};await this.controlTables('pause',failedAt);this.data.transition=null;await this.save();return json({error:'One or more child tables could not resume.',failures,state:publicState(this.data,failedAt)},503)}this.data.transition=null;await this.save();return json(publicState(this.data,now))}finally{this.transitionInFlight=null}
  }
  if(u.pathname==='/end'&&req.method==='POST'){
   if(this.transitionInFlight)return this.busy('end');if(this.data.status==='ended')return json(publicState(this.data));if(!['running','paused','break-pending','break'].includes(this.data.status))return json({error:'Tournament is not active.',state:publicState(this.data)},409);
